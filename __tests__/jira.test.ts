...
describe("getJIRAIssueKeys()", () => {
  ...
  it("gets jira key from different branch names", () => {
    ...
    expect(Jira.getJIRAIssueKeys("feature/newFeature_esch-100")).toEqual([
      "ESCH-100",
    ]);
    expect(Jira.getJIRAIssueKeys("feature/newFeature_ESCH-101")).toEqual([
      "ESCH-101",
    ]);
    expect(Jira.getJIRAIssueKeys("feature/newFeature--mojo-5611")).toEqual([
      "MOJO-5611",
    ]);
    ...
    expect(Jira.getJIRAIssueKeys("chore/task-with-dashes--MOJO-6789")).toEqual([
      "MOJO-6789",
    ]);
    expect(Jira.getJIRAIssueKeys("chore/task_with_underscores--MOJO-6789"))
      .toEqual(["MOJO-6789"]);
    expect(Jira.getJIRAIssueKeys("chore/MOJO-6789-task_with_underscores"))
      .toEqual(["MOJO-6789"]);
    expect(Jira.getJIRAIssueKeys("MOJO-6789/task_with_underscores")).toEqual([
      "MOJO-6789",
    ]);

    expect(Jira.getJIRAIssueKeys("MOJO-6789/task_with_underscores-ES-43"))
      .toEqual(["MOJO-6789", "ES-43"]);
...
describe("getNoIdComment()", () => {
  it("should return the comment content with the branch name", () => {
    expect(Jira.getNoIdComment("test_new_feature")).toContain(
      "test_new_feature",
    );
  });
});
...
describe("getLabelsForDisplay()", () => {
  it("generates label markup without spaces", () => {
    expect(
      ...
      `<a href="url-one" title="one">one</a>, <a href="url-two" title="two">two</a>`,
    );
  });
});
...
describe("JIRA Client", () => {
  ...
  it.skip("should be able to access the issue", async () => {
    const jira = new Jira(
      "https://cleartaxtech.atlassian.net/",
      "<username>",
      "<token_here>",
    );
...
describe("isIssueStatusValid()", () => {
  ...
  it("should return true if issue status validation is not enabled", () => {
    const expectedStatuses = ["In Test", "In Progress"];
    expect(Jira.isIssueStatusValid(false, expectedStatuses, issue))
      .toBeTruthy();
  });
});
...
describe("getInvalidIssueStatusComment()", () => {
  it("should return content with the passed in issue status and allowed statses", () => {
    expect(Jira.getInvalidIssueStatusComment("Assessment", "In Progress"))

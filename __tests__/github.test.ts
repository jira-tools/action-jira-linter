...
describe('shouldSkipBranchLint()', () => {
  it('should recognize bot PRs', () => {
    ...
    expect(GitHub.shouldSkipBranchLint('dependabot/npm_and_yarn/types/react-dom-16.9.6')).toBe(true);
    expect(GitHub.shouldSkipBranchLint('feature/add-dependabot-config')).toBe(false);
    expect(GitHub.shouldSkipBranchLint('feature/add-dependabot-config-OSS-101')).toBe(false);

    expect(GitHub.shouldSkipBranchLint('all-contributors')).toBe(true);
    expect(GitHub.shouldSkipBranchLint('all-contributors/add-ghost')).toBe(true);
    expect(GitHub.shouldSkipBranchLint('chore/add-all-contributors')).toBe(false);
    expect(GitHub.shouldSkipBranchLint('chore/add-all-contributors-OSS-102')).toBe(false);
  });
  ...
  it('should handle custom ignore patterns', () => {
    ...
    expect(GitHub.shouldSkipBranchLint('production-release', customBranchRegex)).toBeTruthy();
    expect(GitHub.shouldSkipBranchLint('master', customBranchRegex)).toBeTruthy();
    expect(GitHub.shouldSkipBranchLint('release/v77', customBranchRegex)).toBeTruthy();

    expect(GitHub.shouldSkipBranchLint('release/very-important-feature', customBranchRegex)).toBeFalsy();
    expect(GitHub.shouldSkipBranchLint('masterful', customBranchRegex)).toBeFalsy();
    expect(GitHub.shouldSkipBranchLint('productionish', customBranchRegex)).toBeFalsy();
    expect(GitHub.shouldSkipBranchLint('fix/production-issue', customBranchRegex)).toBeFalsy();
    expect(GitHub.shouldSkipBranchLint('chore/rebase-with-master', customBranchRegex)).toBeFalsy();
    expect(GitHub.shouldSkipBranchLint('chore/rebase-with-release', customBranchRegex)).toBeFalsy();
    expect(GitHub.shouldSkipBranchLint('chore/rebase-with-release/v77', customBranchRegex)).toBeFalsy();
  });
  ...
  it('should return false for other branches', () => {
    expect(GitHub.shouldSkipBranchLint('feature/awesomeNewFeature')).toBeFalsy();
  });
});
...
describe('getHotFixLabel()', () => {
  ...
  it('should return HOTFIX-PROD for production branch', () => {
    expect(GitHub.getHotfixLabel('production-release')).toEqual(GitHub.labels.hotfixProd);
  });

  it('should return HOTFIX-PRE-PROD for release branch', () => {
    expect(GitHub.getHotfixLabel('release/v')).toEqual(GitHub.labels.hotfixPreProd);
  });

...
/** Regex to check for the hidden marker in PR description to avoid adding jira-linter PR details
 * multiple times. */
export const MARKER_REGEX = new RegExp(HIDDEN_MARKER);

/**
 * Bot branch patters to avoid running jira-linter on.
 */
export const BOT_BRANCH_PATTERNS: RegExp[] = [/^dependabot/, /^all-contributors/];

/**
 * Default branch patterms to skip CI. Skip jira-linter when the HEAD ref matches one of these.
 */
...
/**
 * Default total maximum number of additions after which jira-linter will discourage the PR as it is
